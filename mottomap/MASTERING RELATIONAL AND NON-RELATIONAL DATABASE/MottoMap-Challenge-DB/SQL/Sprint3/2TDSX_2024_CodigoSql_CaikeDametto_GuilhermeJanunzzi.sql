...
-- =============================================================================
-- Este script contém a estrutura completa do banco de dados, carga de dados,
-- funções, procedimentos e trigger para a avaliação da Sprint 3.
-- =============================================================================

-- =============================================================================
-- SEÇÃO 0: LIMPEZA DO AMBIENTE (PARA RE-EXECUÇÃO)
-- =============================================================================
-- Drop de objetos na ordem de dependência para evitar erros.
-- -----------------------------------------------------------------------------
BEGIN
...
/
BEGIN
   EXECUTE IMMEDIATE 'DROP PROCEDURE P_RELATORIO_MOTOS_JSON';
   EXECUTE IMMEDIATE 'DROP PROCEDURE P_RELATORIO_AGREGADO_MOVIMENTACOES';
   EXECUTE IMMEDIATE 'DROP FUNCTION F_RELACIONAL_PARA_JSON';
   EXECUTE IMMEDIATE 'DROP FUNCTION F_VALIDA_COMPLEXIDADE_SENHA';
EXCEPTION
   WHEN OTHERS THEN
...
-- =============================================================================
-- SEÇÃO 2: CARGA DE DADOS (MÍNIMO 5 REGISTROS POR TABELA)
-- =============================================================================
-- Tabelas Auxiliares
...
-- Povoando tabelas de Posição, Problemas, Histórico e Câmeras
...
-- -----------------------------------------------------------------------------
-- FUNÇÃO 1: F_RELACIONAL_PARA_JSON
-- Objetivo: Converter o resultado de uma consulta (cursor) para uma string JSON.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION F_RELACIONAL_PARA_JSON (
...
BEGIN
    ...
    WHILE DBMS_SQL.FETCH_ROWS(v_cursor_id) > 0 LOOP
        ...
        FOR i IN 1..v_col_count LOOP
            ...
            ELSE
                v_json_clob := v_json_clob || '"' || REPLACE(v_col_value, '"', '\"') || '"';
            END IF;
...
EXCEPTION
    ...
    WHEN NO_DATA_FOUND THEN
        ...
        RETURN '[]';
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERRO Inesperado na conversão para JSON: ' || SQLERRM);
...
-- -----------------------------------------------------------------------------
-- FUNÇÃO 2: F_VALIDA_COMPLEXIDADE_SENHA
-- Objetivo: Validar a complexidade de uma senha de acordo com regras de negócio.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION F_VALIDA_COMPLEXIDADE_SENHA (
    p_senha IN VARCHAR2
...
-- -----------------------------------------------------------------------------
-- PROCEDURE 1: P_RELATORIO_MOTOS_JSON
-- Objetivo: Gerar um relatório de motos e suas informações (com JOIN) e
--           exibir o resultado em formato JSON usando a F_RELACIONAL_PARA_JSON.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE P_RELATORIO_MOTOS_JSON (
...
BEGIN
    ...
    -- JOIN entre T_MM_MOTO, T_MM_MODELO, T_MM_STATUS_MOTO
    OPEN v_cursor FOR
        SELECT
            m.CD_MOTO,
            m.DS_PLACA,
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERRO Inesperado no procedimento P_RELATORIO_MOTOS_JSON: ' || SQLERRM);
END P_RELATORIO_MOTOS_JSON;
/

-- -----------------------------------------------------------------------------
-- PROCEDURE 2: P_RELATORIO_AGREGADO_MOVIMENTACOES
-- Objetivo: Calcular e exibir somatórios de valores de movimentação com
--           subtotais por filial e um total geral, sem usar ROLLUP/CUBE.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE P_RELATORIO_AGREGADO_MOVIMENTACOES
IS
...
BEGIN
    ...
    FOR rec IN c_movimentacoes LOOP
        v_row_count := v_row_count + 1;
        -- Quebra de grupo (mudança de filial)
        IF v_filial_ant IS NOT NULL AND rec.NM_FILIAL != v_filial_ant THEN
            DBMS_OUTPUT.PUT_LINE(RPAD('-', 20, '-') || ' | ' || RPAD('-', 30, '-') || ' | ' || RPAD('-', 15, '-'));
            DBMS_OUTPUT.PUT_LINE(RPAD('Subtotal ' || v_filial_ant, 53, ' ') || ' | ' || LPAD(TO_CHAR(v_subtotal, '999,999.00'), 15));
            DBMS_OUTPUT.PUT_LINE('');
            v_subtotal := 0;
        END IF;

        -- Exibe a linha de detalhe
        DBMS_OUTPUT.PUT_LINE(RPAD(rec.NM_FILIAL, 20) || ' | ' || RPAD(rec.DS_TIPO_MOVIMENTACAO, 30) || ' | ' || LPAD(TO_CHAR(rec.VL_SOMADO, '999,999.00'), 15));
    ...
    DBMS_OUTPUT.PUT_LINE(RPAD('TOTAL GERAL', 53, ' ') || ' | ' || LPAD(TO_CHAR(v_total_geral, '999,999.00'), 15));
    DBMS_OUTPUT.PUT_LINE(RPAD('=', 82, '='));
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERRO Inesperado no procedimento P_RELATORIO_AGREGADO_MOVIMENTACOES: ' || SQLERRM);
END P_RELATORIO_AGREGADO_MOVIMENTACOES;
/

-- =============================================================================
-- SEÇÃO 5: TRIGGER DE AUDITORIA
-- =============================================================================

-- -----------------------------------------------------------------------------
-- TRIGGER: TRG_AUDITA_T_MM_MOTO
-- Objetivo: Auditar todas as operações de INSERT, UPDATE e DELETE na tabela T_MM_MOTO.
-- Tabela Alvo: T_MM_MOTO
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- SEÇÃO 6: SCRIPTS DE EXECUÇÃO E DEMONSTRAÇÃO
-- =============================================================================
SET SERVEROUTPUT ON;

-- -----------------------------------------------------------------------------
-- Demonstração da Função 2: F_VALIDA_COMPLEXIDADE_SENHA
-- -----------------------------------------------------------------------------
DECLARE
    v_resultado NUMBER;
BEGIN
    DBMS_OUTPUT.PUT_LINE('--- TESTANDO F_VALIDA_COMPLEXIDADE_SENHA ---');
    v_resultado := F_VALIDA_COMPLEXIDADE_SENHA('SenhaValida123');
    DBMS_OUTPUT.PUT_LINE('Senha "SenhaValida123" é válida? (1=Sim/0=Não): ' || v_resultado);
    
    v_resultado := F_VALIDA_COMPLEXIDADE_SENHA('curta');
    ...
    v_resultado := F_VALIDA_COMPLEXIDADE_SENHA('abc');
END;
/

-- -----------------------------------------------------------------------------
-- Demonstração do Procedure 1: P_RELATORIO_MOTOS_JSON
-- -----------------------------------------------------------------------------
BEGIN
    DBMS_OUTPUT.PUT_LINE(CHR(10) || '--- TESTANDO P_RELATORIO_MOTOS_JSON ---');
    -- Execução com filial que tem dados
    P_RELATORIO_MOTOS_JSON(p_cd_filial => 1);
    
    -- Teste de exceção (filial inexistente)
    DBMS_OUTPUT.PUT_LINE(CHR(10) || '-- Testando exceção de filial inexistente --');
    P_RELATORIO_MOTOS_JSON(p_cd_filial => 999);
END;
/

-- -----------------------------------------------------------------------------
-- Demonstração do Procedure 2: P_RELATORIO_AGREGADO_MOVIMENTACOES
-- -----------------------------------------------------------------------------
BEGIN
    DBMS_OUTPUT.PUT_LINE(CHR(10) || '--- TESTANDO P_RELATORIO_AGREGADO_MOVIMENTACOES ---');
    P_RELATORIO_AGREGADO_MOVIMENTACOES;
END;
/

-- -----------------------------------------------------------------------------
-- Demonstração do Trigger: TRG_AUDITA_T_MM_MOTO
-- -----------------------------------------------------------------------------
DECLARE
